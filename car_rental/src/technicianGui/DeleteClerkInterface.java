...
public class DeleteClerkInterface extends JDialog {
	...
	public DeleteClerkInterface(TechnicianInterfaces techFrame) {
		...
		JLabel defaultLabel = new JLabel("Search the clerk you want to delete");
				...
				if(firstNameField.getText().equalsIgnoreCase("") && lastNameField.getText().equalsIgnoreCase("")) {
					...
					JOptionPane.showMessageDialog(null, "Please don't leave the Last Name field blank!","Error",JOptionPane.ERROR_MESSAGE);
						...
						if(check!=0) {
							...
							resultLabel.setText("Clerk Exist");
							resultLabel.setForeground(Color.ORANGE);
							removeButton.setVisible(true);
						}
						else {
							IdLabel.setText("");
							resultLabel.setText("Clerk doesn't found!");
		...
		removeButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(value==JOptionPane.YES_OPTION) {
					String fname = firstNameField.getText();
					String lname = lastNameField.getText();
					boolean check = techFrame.loginTechnician.deleteClerk(techFrame.clerks, fname, lname);
					if(check==true) {
